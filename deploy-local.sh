...
_themeDestDir="$_projectAbspath/docker/run/wordpress/wp-content/themes/goldeimer"

printf ">> %s\n" "Copying theme files to staging destination"
sudo cp -r "$_projectAbspath/packages/wordpress-theme/dist/"* "$_themeDestDir"
...
sudo chown -R 33:33 "$_themeDestDir"
